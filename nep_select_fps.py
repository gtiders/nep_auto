...
def get_selected_frames(
    fxyz, calc, min_distance=0.05, mean=True, normalized=True, selected=True
):
    ...
    for i, structrue in enumerate(dataset):
        ...
        if mean:
            descriptors.append(
                np.mean(calc.get_property("descriptor", structrue), axis=0)
            )
    ...
    if not mean:
        ...
        print(
            "The total number of atoms in original dataset are {0}.".format(
                all_descriptors.shape[0]
            )
        )
        print(
            "The total number of descriptor components are {0}.".format(
